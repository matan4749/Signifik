    ...
    html: `
      <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
          <tr>
            <td style="padding: 12px; background: #f9fafb; font-weight: 600; width: 40%; border-radius: 8px 0 0 0;">Customer</td>
            <td style="padding: 12px; background: #f9fafb; border-radius: 0 8px 0 0;">${opts.customerName} (${opts.customerEmail})</td>
          </tr>
          <tr>
            <td style="padding: 12px; font-weight: 600;">Business</td>
            <td style="padding: 12px;">${opts.businessName}</td>
          </tr>
          <tr>
            <td style="padding: 12px; background: #f9fafb; font-weight: 600;">Site ID</td>
            <td style="padding: 12px; background: #f9fafb;">${opts.siteId}</td>
          </tr>
          <tr>
            <td style="padding: 12px; font-weight: 600;">Live URL</td>
            <td style="padding: 12px;"><a href="${opts.siteUrl}" style="color: #6366f1;">${opts.siteUrl}</a></td>
          </tr>
        </table>

        <a href="${opts.siteUrl}" style="display: inline-block; padding: 12px 24px; background: #6366f1; color: #fff; text-decoration: none; border-radius: 8px; font-weight: 600;">View Live Site</a>
      </div>
    `,
  });
}
...
export async function sendWelcomeEmail(email: string, name: string): Promise<void> {
  ...
  await sg.send({
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
        <h1 style="color: #6366f1;">Welcome to ${APP_NAME}, ${name}! 👋</h1>
...
export async function sendTrialEndingEmail(email: string, name: string): Promise<void> {
  ...
  await sg.send({
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
        <h1 style="color: #f59e0b;">Trial ending soon, ${name}</h1>
...
export async function sendPaymentFailedEmail(email: string, name: string): Promise<void> {
  ...
  await sg.send({
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
        <h1 style="color: #ef4444;">Payment Failed, ${name}</h1>
...
export async function sendSiteDisabledEmail(email: string, name: string, businessName: string): Promise<void> {
  ...
  await sg.send({
    ...
    html: `
      <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
        <h1 style="color: #6b7280;">Site Paused: ${businessName}</h1>
        <p>Hi ${name}, your site has been paused due to a failed payment. Reactivate your subscription to restore it.</p>
