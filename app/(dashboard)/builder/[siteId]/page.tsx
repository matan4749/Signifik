...
import { subscribeToSite, updateSite } from '@/lib/firebase/firestore';
...
export default function EditSitePage() {
  ...
  // Local edit state — mirrors config
  ...
  useEffect(() => {
    const unsub = subscribeToSite(siteId, (s) => {
      ...
      setSite(s);
      setLoading(false);
      // Populate local state once
      setHeadline(s.config.hero.headline);
      setSubheadline(s.config.hero.subheadline);
      setCtaText(s.config.hero.ctaText);
      setCtaHref(s.config.hero.ctaHref);
      setPhone(s.config.contact.phone || '');
      setWhatsapp(s.config.contact.whatsapp || '');
      setEmail(s.config.contact.email || '');
      setWazeAddress(s.config.contact.wazeAddress || '');
      setFacebookUrl(s.config.contact.facebookUrl || '');
      setInstagramUrl(s.config.contact.instagramUrl || '');
      setSeoTitle(s.config.seo.title);
      setSeoDescription(s.config.seo.description);
      setComponents(s.config.components);
      setPrimaryColor(s.config.design.primaryColor);
      setAccentColor(s.config.design.accentColor);
    });
    return unsub;
  }, [siteId, router]);
  ...
  return (
    <>
          ...
          >
            ...
            {activeSection === 'seo' && (
              <Card className="space-y-4">
                ...
                <Input
                  label={t.editor_page_title}
                  value={seoTitle}
                  onChange={(e) => setSeoTitle(e.target.value)}
                  hint={t.editor_page_title_hint}
                />
                <div>
      ...
      {/* Analytics below the edit panel */}
      <div className="mt-8">
